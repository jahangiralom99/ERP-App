...
const EmployField = ({ selectEmploy, setSelectEmploy }) => {
  ...
  const handleSearch = async () => {
    ...
    const url1 = `${fetchURL}/gets/Employee?erp_url=${url}&filters=${query}&fields=["*"]`;
    try {
  ...
  return (
    <fieldset className="relative border-[1px] border-gray-600 rounded-xl">
      <legend className="ml-3 px-[5px] text-xs text-gray-500">
        Select Employ <span className="text-[#FF0000] text-xl">*</span>
      </legend>
      <div
        onClick={() => {
          setOpen3(!open3);
      ...
      >
        ...
        {open3 && (
          <div
            ...
            className="fixed border border-gray-300 top-[259px] left-1/2 -translate-x-1/2 px-3 bg-white rounded-box z-[1] w-[350px] h-[300px] overflow-hidden p-4 shadow"
          >
            <div>
                  ...
                  return (
                    ...
                    >
                      <input
                        ...
                        checked={selectEmploy === item.name}
                        defaultChecked
                      />
                      <p>{item.name}</p>
                    </div>
